...
def assign_player_colours(initial_dict : dict):
        ...
        while manual not in ["y", "m"]:
                ...
                manual = input("Would you like to use our pre-selected, super aesthetic colours or customise your own? Please be responsible!!" +
                        " Type 'Y' for yes and 'M' to manually select.").lower().strip()
                
        if manual == "m":

                for player in range(player_number):
                        print(f"OKAY! PLAYER {player + 1}, YOU'RE UP!!!")
                        copy_dict[player + 1]["color"] = choose_colours()

        else:
                
                for player in range(player_number):
                        copy_dict[player+1]["color"] = preset_colors[player]

        for player in range(player_number):
                print(ansi_stitching(copy_dict[player + 1]["color"], f"Player {player + 1}, this is your colour."))

                        
        return copy_dict
        
def get_player_number(initial_dict):
        while True:
                try:
        ...
        initial_dict["player_number"] = player_number
...
def set_up_game(game_info : dict):
        ...
        initial_dict = get_player_number(initial_dict)
